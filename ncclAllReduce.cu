...
int main(int argc, char* argv[]) {
  ...
  ncclGroupStart(); 
  	...
  	for(int g = 0; g < nGPUs; g++) {
   	cudaSetDevice(DeviceList[g]);
      ncclAllReduce(x_d_data[g], Sx_d_data[g], data_size, ncclDouble, ncclSum, comms[g], s[g]); /*Reducing x vector*/
      ncclAllReduce(y_d_data[g], Sy_d_data[g], data_size, ncclDouble, ncclSum, comms[g], s[g]); /*Reducing y vector*/
    }
